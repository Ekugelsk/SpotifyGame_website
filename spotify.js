...
(function () {
  ...
  const CLIENT_ID = "840bc8081d624c9c94da6b3da577ef72";
  const CLIENT_SECRET = "d6e33eecbfb5424fbf39e19bb588a5a0";
  ...
  async function fetchArtists(name) {
    ...
    let resp = await fetch(
      SEARCH_EP + `q=${name}&type=artist&limit=${MAX_SEARCH_RESULTS}`,
      {
        headers: {
  ...
  function populateArtistResults(artistsData) {
    ...
    if (artistsData.artists.items.length === 0) {
      ...
      for (let i = 0; i < artistsData.artists.items.length; i++) {
        let item = artistsData.artists.items[i];
        console.log(item);
        document
          .querySelector("#search-results")
          .appendChild(genArtistCard(item));
      }
    }
  }
  ...
  function genArtistCard(artistInfo) {
    ...
    head3.textContent = artistInfo.name;
  ...
  function populateTracks() {
    ...
    for (let i = 0; i < tracks.length; i++) {
      document
        .querySelector("#track-container")
        .appendChild(genTrackCard(tracks[i]));
    }
    // Part 3b: TODO
  }
